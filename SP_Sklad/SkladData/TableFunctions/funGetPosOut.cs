...
namespace SP_Sklad.SkladData
{
    ...
    {
        public static List<GetPosOut_Result> GetPosOut(this BaseEntities db, DateTime from_date, DateTime to_date, int mat_id, int? ka_id)
        //   where T : GetPosOut_Result
        {
           ...
           where  wbl.ondate between {0} and {1} and wbl.checked = 1 and wbl.wtype = -1
                  and {2} in (m.matid , 0)
                  and ka.kaid = {3}
           group by  wbd.PosId, wbl.WbillId, wbl.WType, wbl.Num, wbl.OnDate, wbl.DocId, ka.KaId, ka.Name , w.WID, w.name , m.MatId, m.name , m.Artikul, 
            ...
            return db.Database.SqlQuery<GetPosOut_Result>(sql, from_date, to_date, mat_id, ka_id).ToList();
        }
    }
