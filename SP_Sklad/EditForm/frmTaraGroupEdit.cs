...
{
    public partial class frmTaraGroupEdit : DevExpress.XtraEditors.XtraForm
    {
        int? _grp_id { get; set; }
        int? _pid { get; set; }
        private TaraGroup _mg { get; set; }
        private BaseEntities _db { get; set; }
        private DbContextTransaction current_transaction { get; set; }

        public frmTaraGroupEdit(int? GrpId = null, int? PId = null)
        {
            InitializeComponent();
        ...
        private void frmSvcGroupEdit_Load(object sender, EventArgs e)
        {
            ...
            if (_grp_id == null)
            {
                _mg = _db.TaraGroup.Add(new TaraGroup
                {
                    PId = 0,
                    Name = ""
                });
            ...
            {
                _mg = _db.TaraGroup.Find(_grp_id);
            }
            ...
            if (_mg != null)
            {
                ...
                GrpIdEdit.Properties.TreeList.DataSource = DB.SkladBase().TaraGroup.Select(s => new { s.GrpId, s.PId, s.Name, ImageIndex = 2 }).ToList();

                TaraGroupDS.DataSource = _mg;
            }
        }
        ...
        {
            this.Text = "Група контейнерів: " + textEdit10.Text;
        }
    }
}
